...
El timbre es el atributo perceptual que permite distinguir dos sonidos de igual altura y volumen producidos por fuentes distintas~---~por ejemplo, una misma nota ejecutada en un piano y en una guitarra. La capacidad de generar timbres intermedios entre instrumentos conocidos, comúnmente denominada \emph{morphing} tímbrico, cobra especial relevancia tanto en la producción musical como en el diseño sonoro, dado que permite expandir la paleta acústica más allá de las limitaciones de los instrumentos acústicos tradicionales.

Con el propósito de proveer herramientas para la síntesis de nuevas sonoridades a partir de timbres familiares, resulta de gran interés explorar el potencial generativo de modelos basados en redes neuronales profundas. En particular, este trabajo se enfoca en investigar la factibilidad y eficacia de realizar procesos de interpolación que operen directamente sobre las representaciones tímbricas aprendidas de distintos instrumentos.
...
Posteriormente, orientados al procesamiento en el dominio del audio, Caillon y Esling~\cite{caillon2021rave} demostraron con la arquitectura RAVE que los VAE son capaces de sintetizar audio de alta calidad en tiempo real operando directamente sobre la forma de onda cruda. Por su parte, Riera, Eguía y Zabaljáuregui~\cite{riera2017timbre} exploraron la construcción de espacios tímbricos mediante autoencoders dispersos (\textit{sparse autoencoders}), evidenciando que las representaciones aprendidas capturan las relaciones perceptuales entre timbres instrumentales de manera consistente con la percepción humana.
...
\begin{figure}[H]
\caption{Error de Reconstrucción (MSE) promedio sobre todos los instrumentos vs. dimensión del espacio latente, con desvío estándar.}
\centering
...
\section{Selección del parámetro $\beta$}

Definida la dimensión del espacio latente, resta fijar el valor del coeficiente $\beta$ que pondera el término de divergencia KL en la \textit{loss}. Para ello entrenamos el modelo base con cinco valores de $\beta$~---~$0.01$, $0.001$, $0.0001$, $10^{-5}$ y $0$~---~y analizamos cómo varían el error de reconstrucción y la divergencia KL sobre el conjunto de validación. La figura~\ref{fig:beta_evolution} presenta la evolución suavizada de ambas métricas a lo largo del entrenamiento.

\begin{figure}[H]
\caption{Evolución suavizada (media móvil) del error de reconstrucción (izquierda) y de la divergencia KL (derecha) sobre validación, para los cinco valores de $\beta$.}
\centering
\label{fig:beta_evolution}
\includegraphics[width=\linewidth]{imgs/02_comparacion_beta_vae/validation_smoothed_evolution.png}
\end{figure}

El comportamiento observado responde a la tensión intrínseca del $\beta$-VAE: a medida que $\beta$ disminuye, el error de reconstrucción se reduce, pero la divergencia KL crece~---~es decir, el espacio latente queda menos regularizado~---. Los casos extremos ilustran ambos límites: con $\beta=0.01$ la regularización es tan fuerte que penaliza marcadamente la reconstrucción (error en torno a $0.008$), mientras que con $\beta=0$ la KL crece sin control, dado que ningún término acota la distribución latente. Existe entonces un compromiso entre fidelidad de reconstrucción y regularización del espacio latente que ningún valor optimiza simultáneamente.

Para identificar el mejor balance planteamos este compromiso como una curva de Pareto, ubicando cada modelo según su error de reconstrucción y su divergencia KL promedio en la etapa final del entrenamiento. La figura~\ref{fig:beta_pareto} muestra esta curva en escala lineal~---~excluyendo $\beta=0$ por tener una KL desproporcionadamente alta~---~y en escala logarítmica incluyéndolo.

\begin{figure}[H]
\caption{Compromiso entre error de reconstrucción y divergencia KL para los distintos valores de $\beta$.}
\label{fig:beta_pareto}
\begin{subfigure}{.5\textwidth}
    \caption{Escala lineal (excluyendo $\beta=0$).}
    \centering
    \label{fig:beta_pareto_lin}
    \includegraphics[width=\linewidth]{imgs/02_comparacion_beta_vae/pareto_tradeoff_linear.png}
\end{subfigure}
\begin{subfigure}{.5\textwidth}
    \caption{Escala logarítmica (incluyendo $\beta=0$).}
    \centering
    \label{fig:beta_pareto_log}
    \includegraphics[width=\linewidth]{imgs/02_comparacion_beta_vae/pareto_tradeoff_log.png}
\end{subfigure}
\end{figure}

El valor $\beta=0.001$ se ubica precisamente en el codo de la curva de Pareto: representa el punto a partir del cual seguir reduciendo $\beta$ aporta una mejora marginal en la reconstrucción a costa de un incremento sustancial de la divergencia KL, mientras que aumentarlo hacia $0.01$ degrada la reconstrucción sin un beneficio proporcional en la regularización. En otras palabras, es el valor que mejor equilibra ambos objetivos.

Esta elección numérica fue contrastada además con una evaluación subjetiva: al escuchar los audios reconstruidos por los modelos entrenados con cada $\beta$, los obtenidos con $\beta=0.001$ ofrecen una calidad perceptual comparable a la de los valores más bajos, sin la degradación audible que introduce $\beta=0.01$. En base a este doble criterio~---~cuantitativo y perceptual~---~adoptamos $\beta=0.001$ como configuración del $\beta$-VAE para el resto del trabajo, conservando además la variante $\beta=0$ (AE estándar) como línea de base para las comparaciones posteriores.
...
En ambas configuraciones se observa una separación clara entre los distintos instrumentos, que forman \textit{clusters} diferenciados. La diferencia más notoria es que el modelo con $\beta=0.001$ presenta una nube de puntos visiblemente más suavizada y compacta, consistente con el efecto regularizador que la divergencia KL impone sobre el espacio latente; el modelo con $\beta=0$, en cambio, dispone los \textit{clusters} de forma más dispersa.

Para cuantificar esta separación de manera objetiva utilizamos el estadístico $U$ de Wilcoxon-Mann-Whitney en su versión multidimensional, calculado sobre los vectores latentes de cada par de instrumentos. La métrica se normaliza al intervalo $[0, 1]$, donde un valor cercano a $1$ indica que ambas distribuciones están fuertemente separadas en el espacio latente y un valor cercano a $0.5$ indica solapamiento total. Los resultados se sintetizan en la matriz de la figura~\ref{fig:umap_sep}.

\begin{figure}[H]
\caption{Matriz de separación entre instrumentos en el espacio latente del modelo base, medida con el estadístico $U$ de Wilcoxon-Mann-Whitney multidimensional (valores cercanos a $1$ indican mayor separación).}
\label{fig:umap_sep}
\begin{subfigure}{.5\textwidth}
    \caption{$\beta = 0.001$.}
    \centering
...
La matriz confirma cuantitativamente lo que sugiere la visualización: ambos enfoques logran una separación clara entre instrumentos, con valores de $U$ próximos a $1$ en todos los pares. Resulta interesante que el modelo sin regularización ($\beta=0$) alcanza incluso una separación algo mayor~---~con un máximo de $0.926$ para el par bajo-voz~---, lo cual es coherente con un espacio latente más disperso. Sin embargo, esta mayor separación no es necesariamente deseable para nuestro objetivo: un espacio latente menos regularizado tiende a perjudicar las interpolaciones, ya que la falta de suavidad dificulta las transiciones graduales entre instrumentos. Esta hipótesis será puesta a prueba en la sección siguiente.
...
Para descartar que el comportamiento observado sea particular del piano como destino, repetimos el análisis tomando sucesivamente cada instrumento como destino y los tres restantes como origen. Promediando la similitud FAD sobre todas las combinaciones y muestras de $Z$, obtenemos la comparación agregada de la Figura~\ref{fig:sim_aggregated_all_instruments}.
...
El patrón se mantiene de forma consistente a lo largo de todas las combinaciones de instrumentos de origen y destino: la configuración \textit{Checkpoint-$\beta$-VAE} es sistemáticamente la de mejor desempeño. Tanto incorporar únicamente la regularización ($\beta>0$) como partir de un \textit{checkpoint} común mejoran la similitud respecto del peor caso (\textit{Scratch-AE}), lo que confirma que ambos factores contribuyen de manera independiente a la calidad de la interpolación, siendo la regularización del espacio latente el de mayor impacto.
...
\includegraphics[scale=0.5]{imgs/04_similitud/fad_random_latent_gaussian_piano_to_guitar.png}
\end{figure}
