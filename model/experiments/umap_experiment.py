...
def sample_latents_by_class(
    ...
    for class_id in classes:
        ...
        if (
            max_frames_per_class is not None
            and len(class_indices) > max_frames_per_class
        ):
            class_indices = rng.choice(
                class_indices, size=max_frames_per_class, replace=False
            )
    ...
    if max_total_frames is not None and len(selected_indices) > max_total_frames:
        selected_indices = rng.choice(
            selected_indices, size=max_total_frames, replace=False
        )
...
def run_model_base_comparison(
        ...
        for j in range(i + 1, len(instrument_names)):
            ...
            if len(Z_pair) > max_stats_frames_per_pair:
                sampled_indices = rng.choice(
                    len(Z_pair), size=max_stats_frames_per_pair, replace=False
                )
    ...
    ax.set_title("Base Model (Beta): Trayectorias Latentes por Instrumento")
    ax.set_xlabel("UMAP 1")
    ax.set_ylabel("UMAP 2")
    ax.grid(True, alpha=0.3)
    ax.legend()
